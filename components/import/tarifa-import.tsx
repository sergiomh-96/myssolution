...
export function TarifaImport() {
  ...
  const [tarifaFechaInicio, setTarifaFechaInicio] = useState('')
  const [tarifaFechaFin, setTarifaFechaFin] = useState('')
  ...
  async function handleImport() {
    if (!rows.length || !tarifaNombre.trim()) return
    ...
    try {
      const res = await fetch('/api/import/tarifas', {
        ...
        body: JSON.stringify({
          tarifaNombre: tarifaNombre.trim(),
          tarifaFechaInicio: tarifaFechaInicio || undefined,
          tarifaFechaFin: tarifaFechaFin || undefined,
          rows,
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Error desconocido')
      setResult({ tarifaId: data.tarifaId, inserted: data.inserted, notFound: data.notFound ?? [] })
    } catch (err) {
  ...
  const canImport = hasRequired && tarifaNombre.trim().length > 0 && !result
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-lg bg-muted/20">
        <div className="space-y-1.5">
          <Label htmlFor="tarifa-nombre" className="text-xs font-medium">
            Nombre de tarifa <span className="text-destructive">*</span>
          </Label>
          <Input
            id="tarifa-nombre"
            ...
            className="text-sm"
          />
          <p className="text-[11px] text-muted-foreground">Si ya existe, se actualizarán sus precios</p>
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="fecha-inicio" className="text-xs font-medium">Fecha inicio</Label>
          <Input
            id="fecha-inicio"
            type="date"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="fecha-fin" className="text-xs font-medium">Fecha fin</Label>
          <Input
            id="fecha-fin"
            type="date"
            value={tarifaFechaFin}
            onChange={e => setTarifaFechaFin(e.target.value)}
            className="text-sm"
          />
        </div>
      </div>
      ...
      {!file && (
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Formatos: .xlsx, .xls, .csv — Columnas: referencia, precio</p>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-green-700 bg-green-50 border border-green-200 rounded-md px-4 py-3">
            ...
            <span className="text-sm font-medium">
              {result.inserted} precios importados/actualizados en la tarifa (ID: {result.tarifaId}).
            </span>
          </div>
          {result.notFound.length > 0 && (
            <div className="flex items-start gap-2 text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-4 py-3">
              ...
              <div className="text-sm">
                <strong>{result.notFound.length} referencias no encontradas en Productos:</strong>
                <span className="ml-1 font-mono text-xs">{result.notFound.join(', ')}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {!tarifaNombre.trim() && file && (
          <p className="text-xs text-muted-foreground">Introduce el nombre de la tarifa para continuar</p>
        )}
        {result && (
